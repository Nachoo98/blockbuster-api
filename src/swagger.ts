...
function operationsSorter(a: IOperations, b: IOperations) {
    const httpMethodOrder: Record<string, number> = { get: 1, post: 2, patch: 3, put: 4, delete: 5 };

    const getMethodAndPath = (operation: IOperations) => ({
        method: httpMethodOrder[operation._root.entries.find(entry => entry[0] === 'method')?.[1]?.toLowerCase() ?? ''] ?? 99,
        path: operation._root.entries.find(entry => entry[0] === 'path')?.[1] ?? '',
    });
